...
const ModalNewProject = ({ isOpen, onClose, onProjectCreated }) => {
  ...
  useEffect(() => {
    const allPhases = availablePhases.map((t) => t.value);

    const abntRequiredPhases = [
      "capa",
      "folha_de_rosto",
      "resumo",
      "abstract",
    ...
    if (
      JSON.stringify(formData.phases) !== JSON.stringify(newPhases)
    ) {
      setFormData((prev) => ({
        ...prev,
        phases: newPhases,
  ...
  }, [formData.type, t, availablePhases]);
  ...
  const handleAddCustomPhase = () => {
    const trimmedPhase = newPhase.trim();

    if (!trimmedPhase) return;

    const alreadyExists = availablePhases.some(
      (phase) =>
        phase.value.toLowerCase() ===
        trimmedPhase.toLowerCase()
    );
    ...
    const customPhaseObject = {
      value: trimmedPhase,
      label: trimmedPhase,
      isCustom: true,
    };

    setAvailablePhases((prev) => [
      ...prev,
      customPhaseObject,
    ]);
    ...
    setFormData((prev) => ({
      ...prev,
      phases: [...prev.phases, trimmedPhase],
    }));
  ...
  const removerColaborador = (email) => {
    handleCollaboratorChange(
      formData.collaborators.filter((e) => e !== email)
    );
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      errors.name = t("messages.projectNameRequired");
    }
  ...
  const nextStep = () => {
    ...
    if (currentStep === 1) {
      if (allErrors.name) {
        currentStepErrors.name = allErrors.name;
      }
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData({
        ...
        description: "",
        type: "TCC",
        phases: availablePhases.map((t) => t.value),
        collaborators: [],
        startDate: "",
        endDate: "",
      });
        ...
        <div className="modal-body">
          ...
          >
            ...
            {currentStep === 3 && (
              <div>
                <h3>
                  {t("titles.step3Review")}
                </h3>

                <p className="review-intro-message">
                  {t(
                    "messages.reviewProjectDetails"
                  )}
                </p>
